    ...
    def __init__(self, startDir) :
        ...
        for (path, dirlist, filelist) in os.walk(startDir) :
            for file in filelist :
                self._filePath.append( (path, os.path.join(path,file)))  
                       
        
    def searchName(self, stopEvent, regex, searchStr, resultList) :
        ...
        for (filePath, filename) in self._filePath :
            ...
            if search(regex, filename) and ( len(searchStr) == 0 or strIsInFile(searchStr, filename) ):
